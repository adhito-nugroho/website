...
document.addEventListener("DOMContentLoaded", function() {
  ...
  anchorLinks.forEach(link => {
    link.addEventListener('click', function(e) {
      ...
      const isInternalLink = href.startsWith('#') || href.includes(window.location.pathname + '#');
...
function initializeCharts(useCustomData = false) {
  ...
  if (forestAreaChart) {
    ...
    const labels = useCustomData && window.forestAreaChartData ? 
      window.forestAreaChartData.labels : 
      ['Hutan Produksi', 'Hutan Lindung', 'Hutan Rakyat', 'Hutan Kota'];
    
    const data = useCustomData && window.forestAreaChartData ? 
  ...
  if (forestProductionChart) {
    // Get data from global variable if available
    const labels = useCustomData && window.forestProductionChartData ? 
      window.forestProductionChartData.labels : 
      ['Jan', 'Feb', 'Mar', 'Apr', 'Mei', 'Jun'];
    
    const data = useCustomData && window.forestProductionChartData ? 
  ...
  document.querySelectorAll('[data-chart-type]').forEach(button => {
    ...
    newButton.addEventListener('click', function() {
      ...
      } else if (targetChart === 'forestProductionChart' && window.forestProductionChartInstance) {
...
function initializeMonitoringCharts(useCustomData = false) {
  ...
  // Default data
  const defaultData = {
    labels: ['Rehabilitasi Hutan', 'Perhutanan Sosial', 'Perlindungan Hutan', 'Produksi Hasil Hutan'],
    datasets: [{
  ...
  if (useCustomData && window.achievementChartData) {
    console.log('Using custom achievement chart data:', window.achievementChartData);
    
    // Pastikan data tidak kosong
    if (window.achievementChartData.labels && window.achievementChartData.labels.length > 0 && 
      ...
      console.warn('Custom achievement data is empty or invalid, using default data');
    }
  } else {
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  setTimeout(() => {
    ...
    if (typeof AOS === 'undefined') {
      loadScriptAsync('https://cdnjs.cloudflare.com/ajax/libs/aos/2.3.4/aos.js', function() {
    ...
    if (typeof ScrollReveal === 'undefined' && document.querySelector('[data-scroll]')) {
